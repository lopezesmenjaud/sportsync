...
export default function WatchPanel({ sport, competitionKey, competitionName, isOpen }) {
  ...
  return (
    <div style={{ borderTop: '1px solid #e8e8e8', padding: '16px 20px', background: '#f9fafb' }}>

      {/* Encabezado */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 500, color: '#666666', textTransform: 'uppercase', letterSpacing: '0.8px' }}>
          Dónde ver este partido
        </div>
        {!changingCountry && country && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontSize: 12, color: '#666666' }}>📍 {country}</span>
            <button
      ...
      {changingCountry && (
        <div style={{ marginBottom: 14 }}>
          <select
            ...
            style={{ width: '100%', padding: '8px 12px', borderRadius: 8, border: '1px solid #e8e8e8', fontSize: 13, color: '#1C2430', background: '#ffffff', outline: 'none', cursor: 'pointer' }}
          >
      ...
      {/* Cargando */}
      {loading && (
        <div style={{ fontSize: 13, color: '#666666', padding: '4px 0' }}>
      ...
      {!loading && hasAny && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {broadcasts.freeTV?.length > 0 && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                <span style={{ fontSize: 11, fontWeight: 500, color: '#666666', textTransform: 'uppercase', letterSpacing: '0.6px' }}>TV abierta</span>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {broadcasts.freeTV.map(ch => (
                  <span key={ch} style={{ background: '#ffffff', border: '1px solid #e8e8e8', borderRadius: 20, padding: '5px 12px', fontSize: 12, color: '#1C2430', fontWeight: 500 }}>
                    {ch}
                  </span>
                ))}
              </div>
          ...
          {broadcasts.paidTV?.length > 0 && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                <span style={{ fontSize: 11, fontWeight: 500, color: '#666666', textTransform: 'uppercase', letterSpacing: '0.6px' }}>TV de paga</span>
          ...
          {broadcasts.streaming?.length > 0 && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                <span style={{ fontSize: 11, fontWeight: 500, color: '#666666', textTransform: 'uppercase', letterSpacing: '0.6px' }}>Streaming</span>
