    ...
    <>
      ...
      <div style={{ marginTop: 'auto', padding: '0 4px', borderTop: '1px solid rgba(255,255,255,0.15)' }}>
        ...
        {!cargando && google && (google.connected && !google.needsReauth && google.hasCalendarScope ? (
          ...
          <>
          {/* Dos estados distintos, no uno: quien ya conectó su cuenta pero no marcó la casilla
              del calendario NO está "no conectado". Decírselo así lo manda a repetir el mismo
              camino. La rama de "no conectado de verdad" se queda igual. */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '2px 8px 6px', fontSize: 10, color: '#F18006' }}>
            <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#F18006', flexShrink: 0 }} />
            {faltaPermisoCalendario(google) ? 'Falta permiso del calendario' : 'Google Calendar no conectado'}
          </div>
          ...
          >
            ...
            {faltaPermisoCalendario(google) ? 'Dar permiso' : 'Conectar Calendar'}
          </button>
          </>
        ))}
