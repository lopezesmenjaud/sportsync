...
export default function MatchDetail() {
  ...
  if (loading) return (
    <div style={{ background: '#1C2430', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ color: '#8899AA', fontSize: 14 }}>Cargando partido...</div>
    </div>
  )

  if (error || !match) return (
    <div style={{ background: '#1C2430', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
      <div style={{ color: '#8899AA', fontSize: 14 }}>{error || 'Partido no encontrado'}</div>
      <button onClick={() => navigate('/upcoming')} style={{ background: '#1E3A5F', color: '#8899AA', border: 'none', borderRadius: 20, padding: '8px 20px', fontSize: 13, cursor: 'pointer' }}>
        ← Volver a partidos
      </button>
    </div>
  )
  ...
  return (
    <div style={{ background: '#1C2430', minHeight: '100vh', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '24px 20px' }}>

        {/* Back */}
        <button onClick={() => navigate(-1)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#8899AA', background: 'none', border: 'none', cursor: 'pointer', marginBottom: 24, padding: 0 }}>
          ← Volver
        </button>

        {/* Card principal */}
        <div style={{ background: '#142238', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 20, overflow: 'hidden' }}>

          {/* Header */}
          <div style={{ padding: '20px 24px', borderBottom: '0.5px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
          ...
          <div style={{ padding: '32px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, flex: 1 }}>
              <div style={{ width: 64, height: 64, borderRadius: 16, background: '#1E3A5F', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 500, color: '#8899AA' }}>
                {homeInitials}
              </div>
              <div style={{ fontSize: 16, fontWeight: 500, color: '#FFFFFF', textAlign: 'center' }}>{match.homeParticipantName}</div>
            </div>
            <div style={{ padding: '0 20px', fontSize: 24, fontWeight: 600, color: 'rgba(255,255,255,0.15)' }}>vs</div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, flex: 1 }}>
              <div style={{ width: 64, height: 64, borderRadius: 16, background: '#1E3A5F', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 500, color: '#8899AA' }}>
                {awayInitials}
              </div>
              <div style={{ fontSize: 16, fontWeight: 500, color: '#FFFFFF', textAlign: 'center' }}>{match.awayParticipantName}</div>
            </div>
          </div>
          ...
          {(match.venueName || match.city) && (
            <div style={{ textAlign: 'center', paddingBottom: 24 }}>
              ...
              {match.city && <div style={{ fontSize: 12, color: '#8899AA', marginTop: 4 }}>{match.city}{match.country ? `, ${match.country}` : ''}</div>}
            </div>
          )}
        </div>

        {/* Dónde verlo */}
        <div style={{ background: '#142238', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 20, overflow: 'hidden', marginTop: 16 }}>
          <div style={{ padding: '16px 24px', fontSize: 13, fontWeight: 500, color: '#8899AA', textTransform: 'uppercase', letterSpacing: '0.8px', borderBottom: '0.5px solid rgba(255,255,255,0.06)' }}>
            📺 Dónde verlo
          </div>
          <WatchPanel
            sport={match.sport}
        ...
        {/* Resumen IA */}
        <div style={{ background: '#142238', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 20, overflow: 'hidden', marginTop: 16, padding: '20px 24px' }}>
          ...
          <div style={{ background: 'rgba(16,177,199,0.06)', border: '0.5px solid rgba(16,177,199,0.15)', borderRadius: 12, padding: '14px 16px' }}>
            ...
            {loadingSummary
              ? <div style={{ fontSize: 13, color: '#8899AA' }}>Generando resumen...</div>
        ...
        <div style={{ background: '#142238', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 20, overflow: 'hidden', marginTop: 16, padding: '20px 24px' }}>
          ...
          {loadingTickets ? (
            <div style={{ fontSize: 13, color: '#8899AA' }}>Buscando boletos...</div>
          ) : tickets ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {tickets.ticketmasterEvents?.length > 0 ? (
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: '#8899AA', marginTop: 2 }}>{tickets.keyword}</div>
                  </div>
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: 11, color: '#8899AA', marginTop: 2 }}>Reventa y segunda mano</div>
                </div>
                <span style={{ fontSize: 11, fontWeight: 500, color: '#a855f7', flexShrink: 0 }}>StubHub →</span>
              </a>
            </div>
          ) : (
            <div style={{ fontSize: 13, color: '#8899AA' }}>No se pudieron cargar las opciones de boletos.</div>
          )}
        </div>
