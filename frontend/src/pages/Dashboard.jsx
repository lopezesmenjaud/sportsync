        ...
        {!googleCargando && googleStatus && !googleStatus.needsReauth && (!googleStatus.connected || !googleStatus.hasCalendarScope) && (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              {/* Dos estados distintos, no uno: quien ya conectó su cuenta pero no marcó la
                  casilla del calendario NO está "no conectado". El banner tiene que nombrar la
                  casilla, que es lo único que falta. La rama de "no conectado" se queda igual. */}
              <div>
                <div style={{ fontSize: 14, fontWeight: 500, color: '#111827' }}>
                  {faltaPermisoCalendario(googleStatus) ? 'Falta el permiso del calendario' : 'Conecta tu Google Calendar'}
                </div>
                <div style={{ fontSize: 12, color: '#6b7280', marginTop: 2 }}>
                  {faltaPermisoCalendario(googleStatus)
                    ? 'Ya conectaste tu cuenta, pero faltó marcar la casilla del calendario. Sin ella no podemos agendarte los partidos.'
            ...
            >
              ...
              {faltaPermisoCalendario(googleStatus) ? 'Dar permiso' : 'Conectar con Google'}
            </button>
          </div>
        )}
