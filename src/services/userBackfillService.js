...
const { getMatchesForUser } = require("./subscriptionMatchService");
...
async function getMissingEventsForUser(userId) {
  ...
  return { relevant, existing, missingAll, missing, fromDate, toDate, discardedPast, discardedFuture, discardedNoDate };
}
...
async function backfillUserEvents(userId, opts = {}) {
  ...
  const { missing } = await getMissingEventsForUser(userId);
  ...
  for (const m of toProcess) {
    try {
      let res;
      try {
        res = await createCalendarEventIfMissing({ userId, calendarId, match: m });
      } catch (e) {
        if (isRateLimit(e)) {
          onLog("ratelimit", m, e);
